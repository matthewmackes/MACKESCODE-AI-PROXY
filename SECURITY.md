...
The V2 console accepts `?token=...` only as a browser bootstrap convenience. The frontend stores a discovered token in browser storage, scrubs the token from the address bar with `history.replaceState`, and sends API requests with the `x-matts-console-token` header. Operators can also paste the console token into the in-app Sign In dialog when opening a scrubbed URL in a new browser/profile. Native browser WebSocket clients cannot set arbitrary headers, so terminal/TUI WebSocket routes still accept a query-token compatibility fallback; keep token auth enabled for any network-facing use and treat copied WebSocket URLs as sensitive.
