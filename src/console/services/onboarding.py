...
class OnboardingChecklistService:
    ...
    def checks(self, completed):
        ...
        return [
            ...
            self.item(completed, "release_smoke", "Release and browser smoke readiness", "passed" if release["commands_present"] and release["coverage_present"] else "warning", "Release check artifacts and smoke commands are available." if release["coverage_present"] else "Run scripts/release-check.sh to create fresh release and browser smoke evidence.", "run_release_check", release),
    ...
    def release_status(self):
        ...
        commands = ["scripts/release-check.sh", "scripts/browser-smoke.py", "scripts/v2-browser-smoke.py", "scripts/runtime-state.py"]
